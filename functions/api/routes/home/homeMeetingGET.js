...
const users = await homeDB.postAllGET(client);
const count = await homeDB.countGET(client);

const response = {
    title: users.title,
    //usernames: users.map(user => user.uname)
    usernames:users.uname,
    count
}
// 성공적으로 users를 가져왔다면, response를 보내줍니다.
res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.READ_ALL_USERS_SUCCESS, response));
